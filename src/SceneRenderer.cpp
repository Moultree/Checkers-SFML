...
SceneRenderer::SceneRenderer(RenderTarget &_target, Game *_game) : target(&_target), game(_game)
{
...
{
    boardTexture.loadFromFile(getResourcePath("board.png"));
...
void SceneRenderer::drawPieces()
{
    for (int row = 0; row < 8; row++)
    {
        ...
        {
            if (game->getPiece(row, col) == nullptr)
                continue;
...
{
    font.loadFromFile(getResourcePath("minecraft.ttf"));
    text.setFont(font);
    text.setCharacterSize(12);
    text.setFillColor(Color::Black);
    string remainingCount = game->getRemainingCount(type) < 10 ? "0" + to_string(game->getRemainingCount(type))
                                                               : to_string(game->getRemainingCount(type));
...
void SceneRenderer::drawMoves()
{
    ...
    Texture t;
    Sprite shadow;
    t.loadFromFile(getResourcePath("move.png"));
...
{
    forWinningTexture.loadFromFile(getResourcePath("winning.png"));
...
{
    return mousePosition.x >= PLAY_BUTTON_LEFT_CORNER.x && mousePosition.x <= PLAY_BUTTON_RIGHT_CORNER.x &&
           mousePosition.y >= PLAY_BUTTON_LEFT_CORNER.y && mousePosition.y <= PLAY_BUTTON_RIGHT_CORNER.y;
}

bool SceneRenderer::isOnResetButton(Vector2i mousePosition)
{
    return mousePosition.x >= RESET_BUTTON_LEFT_CORNER.x && mousePosition.x <= RESET_BUTTON_RIGHT_CORNER.x &&
           mousePosition.y >= RESET_BUTTON_LEFT_CORNER.y && mousePosition.y <= RESET_BUTTON_RIGHT_CORNER.y;
}
